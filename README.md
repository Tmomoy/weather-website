...
- 雷達回波定時更新
- GPS 定位與明確錯誤狀態
- REST JSON API 與前端非同步查詢，不需重新載入頁面
...
- `GET /api/v1/locations`：取得可搜尋地區
- `GET /api/v1/weather?city=臺北市`：取得目前、36 小時與一週預報 JSON

首頁透過同源 API 非同步更新天氣結果；`/weather` 仍保留完整伺服器渲染頁面，供無 JavaScript 環境與分享網址使用。
